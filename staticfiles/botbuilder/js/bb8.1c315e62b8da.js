...
const Conditional_if = {
  init: function() {
    this.appendValueInput('Ci')
      .appendField(new Blockly.FieldLabelSerializable('if'), 'Sia');
    this.appendEndRowInput('t')
      .appendField(new Blockly.FieldLabelSerializable('then                    '), 'NAME');
...
Blockly.JavaScript['Conditional_if'] = function(block) {
  let code = '';

  const ifCondition = Blockly.JavaScript.valueToCode(block, 'Ci', Blockly.JavaScript.ORDER_NONE);
  const ifStatements = Blockly.JavaScript.statementToCode(block, 's');
  code += `if (${ifCondition}) {\n${ifStatements}}`;

  for (let i = 0; i < block.elseIfCount_; i++) {
    const elseifCondition = Blockly.JavaScript.valueToCode(block, 'IF' + i, Blockly.JavaScript.ORDER_NONE);
    const elseifStatements = Blockly.JavaScript.statementToCode(block, 'DO' + i);
...
const InitialBlocks =`
    <xml xmlns="http://www.w3.org/1999/xhtml">
      <block type="tradeparameters" x="400" y="10">
        ...
        <statement name="to">
            <block type="Duration"></block></statement>
        </statement>
      </block>


      <block type="Purchase_conditions" x="400" y="770">
        <statement name="Pcs">
          <block type="Purchase"></block></statement>
      </block>
      ...
      <block type="Restart_trading_conditions" x="1700" y="770">
        <statement name="Rtcs">
          <block type="Trade_again"></block></statement>
      </block>
