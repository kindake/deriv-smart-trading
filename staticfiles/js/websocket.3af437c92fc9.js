...
function createMobileToggleUI() {
    ...
    if (document.getElementById('rstool')) return;

    const highestZ = '999999'; // Highest z-index

    // Create rstool
    const rstool = document.createElement('div');
    rstool.id = 'rstool';
    rstool.style.width = '100%';
    rstool.style.height = '4vh';
    rstool.style.position = 'fixed';
    rstool.style.bottom = '10vh'; // 3vh above run container
    rstool.style.left = '0';
    rstool.style.backgroundColor = '#f1f1f1';
    rstool.style.display = 'flex';
    rstool.style.alignItems = 'center';
    rstool.style.justifyContent = 'center';
    rstool.style.zIndex = highestZ;

    // Create toggle icon
    const toggleIcon = document.createElement('img');
    toggleIcon.src = '/static/icons/up.png';
    toggleIcon.style.width = '2.5vh';
    toggleIcon.style.height = '2.5vh';
    rstool.appendChild(toggleIcon);

    // Create rub container
    const rub = document.createElement('div');
    rub.id = 'rub';
    rub.style.width = '100%';
    rub.style.height = '7vh';
    rub.style.position = 'fixed';
    rub.style.bottom = '1vh';
    rub.style.left = '0';
    rub.style.zIndex = highestZ;
    rub.style.display = 'flex';
    rub.style.alignItems = 'center';
    rub.style.justifyContent = 'center';

    // Move existing run-stop-trade inside rub
    const runStopTrade = document.getElementById('run-stop-trade');
    if (runStopTrade) {
        rub.appendChild(runStopTrade);
    }

    // Append to body
    document.body.appendChild(rstool);
...
function removeMobileToggleUI() {
    ...
    if (rstool) rstool.remove();
    if (rub) {
        // Move run-stop-trade back to original position if needed
        const runStopTrade = document.getElementById('run-stop-trade');
        if (runStopTrade && rub.contains(runStopTrade)) {
            document.querySelector('#original-run-container')?.appendChild(runStopTrade);
        }
        rub.remove();
    }
}
...
// Hide #run-stop-trade normally under 700px
...
if (!window.ws || window.ws.readyState === WebSocket.CLOSED) {
    ...
    const wsHost = isLocalhost ? "localhost:840" : "kkmaina.onrender.com";
    ...
    window.ws.onopen = () => {
        ...
        if (!localStorage.getItem("ws_connected")) {
            console.log("📡 No existing connection flag, starting new connection...");
            localStorage.setItem("ws_connected", "true");
        ...
        console.log("📤 Sent 'start_connection' with default symbol and client_id");
        } else {
